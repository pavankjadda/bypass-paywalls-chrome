let arr_localstorage_hold = ['sfchronicle.com', 'cen.acs.org'];
...
if (matchDomain('rep.repubblica.it')) {
  if (location.href.includes('/pwa/')) {
    setTimeout(function () {
      window.location.href = window.location.href.replace('/pwa/',
        '/ws/detail/');
    }, 400);
  }
...
if (matchDomain('telegraaf.nl')) {
  ...
  let spotx_banner = document.querySelector(
...
if (matchDomain('washingtonpost.com')) {
  if (window.location.href.includes('/gdpr-consent/')) {
    const free_button = document.querySelector(
    ...
    setTimeout(function () {
      ...
      if (gdprcheckbox) {
        ...
        document.querySelector(
          '.gdpr-consent-container .consent-page:not(.hide) .continue-btn.button.accept-consent').click();
      }
...
if (matchDomain('afr.com')) {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0; i < hidden_image.length; i++) {
      let src = hidden_image[i].src;
      if ('src: ' + src.indexOf(".gif") !== -1) {
        let data_src = hidden_image[i].getAttribute("data-src");
        if (data_src) {
...
if (matchDomain(['parool.nl', 'trouw.nl', 'volkskrant.nl', 'demorgen.be'])) {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const hidden_section = document.querySelector(
...
if (matchDomain('bloombergquint.com')) {
  const articlesLeftModal = document.getElementsByClassName(
    'paywall-meter-module__story-paywall-container__1UgCE')[0];
...
if (matchDomain('medium.com')) {
  ...
  if (DOMElementsToTextDiv[2]) {
    removeDOMElement(DOMElementsToTextDiv[2]);
  }
}
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0; i < hidden_image.length; i++) {
      ...
      if (src.indexOf("data:image/gif") !== -1) {
        ...
        let data_bg = hidden_image[i].getAttribute("data-bg");
        if (data_bg) {
...
if (matchDomain(
  ['examiner.com.au', 'thecourier.com.au', 'theadvocate.com.au'])) {
...
if (matchDomain('lemonde.fr')) {
  document.addEventListener('DOMContentLoaded', () => {
    const hidden_section = document.getElementsByClassName(
      'article__content--restricted-media')[0];
    if (hidden_section) {
      hidden_section.classList.remove('article__content--restricted-media');
    }
    const longform_article_restricted = document.getElementsByClassName(
      'article__content--restricted')[0];
    if (longform_article_restricted) {
      longform_article_restricted.classList.remove(
        'article__content--restricted');
    }
    const longform_paywall = document.getElementsByClassName(
      'paywall--longform')[0];
    if (longform_paywall) {
      longform_paywall.classList.remove('paywall--longform');
    }
    const paywall = document.getElementById('js-paywall-content');
    const friend_paywall = document.getElementsByClassName(
      'friend--paywall')[0];
...
if (matchDomain('caixinglobal.com')) {
  ...
  if (appContent) {
    const p_hidden = document.querySelectorAll(
      'p:not([style="display:block;"]');
